...
class sortAlgorithms {
    ...
    HeapSort = async () => {
        ...
        await this.HeapDivider(i, this.size);
        }
    ...
    HeapDivider = async (rootIndex, size) => {
        let largest = rootIndex;
        let leftChild = 2 * rootIndex + 1;
        let rightChild = 2 * rootIndex + 2;
    
        if (leftChild < size && (await this.compare(leftChild, largest))) {
        largest = leftChild;
        }
    
        if (rightChild < size && (await this.compare(rightChild, largest))) {
        largest = rightChild;
        }
    
        if (largest !== rootIndex) {
        await this.help.swap(rootIndex, largest);
        await this.HeapDivider(largest, size);
        }
    }
    
    compare = async (indexA, indexB) => {
        let valueA = Number(this.list[indexA].getAttribute("value"));
        let valueB = Number(this.list[indexB].getAttribute("value"));
        
        await this.help.mark(indexA);
        await this.help.mark(indexB);
    
        if (valueA > valueB) {
        await this.help.unmark(indexA);
        await this.help.unmark(indexB);
        return true;
        } else {
        await this.help.unmark(indexA);
        await this.help.unmark(indexB);
